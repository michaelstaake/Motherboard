...
$hideSubtotals = ($settings['inventory_hide_subtotals'] ?? '0') === '1';
ob_start();
?>
...
<div class="py-8">
    ...
    <div class="bg-white shadow rounded-lg">
        <form method="POST" action="<?= BASE_URL ?>/module-manager/inventory/settings" class="px-6 py-4 space-y-6">
            ...
            <div class="flex items-start">
                <input id="inventory_hide_subtotals" name="inventory_hide_subtotals" type="checkbox" value="1" <?= $hideSubtotals ? 'checked' : '' ?> class="h-4 w-4 mt-0.5 text-primary-600 focus:ring-primary-500 border-gray-300 rounded">
                <div class="ml-2">
                    <label for="inventory_hide_subtotals" class="block text-sm text-gray-700"><?= t('inventory.hide_subtotals') ?></label>
                    <p class="mt-1 text-sm text-gray-500"><?= t('inventory.hide_subtotals_help') ?></p>
                </div>
            </div>
